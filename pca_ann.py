...
def fit_model(x_train, y_train, epochs = 100, batch_size = 1024):
    ...
    ts = Sequential([
        ...
        # 1st set of layers (128 -> 512)
        Dense(units = 128, kernel_initializer = GlorotNormal()),
        BatchNormalization(),
        Activation(tf.keras.activations.relu),
        Dropout(0.1),
        Dense(units = 512, kernel_initializer = GlorotNormal()),
        BatchNormalization(),
        Activation(tf.keras.activations.relu),
        Dropout(0.2)
        ])
    ...
    # 3rd set of layers (1024 -> 256 -> 64)
    model_concat = Dense(units = 1024, kernel_initializer = GlorotNormal())(model_concat)
    ...
    # fit model
    opt = SGD(learning_rate = 0.5, momentum = 0.99, decay = 0.0005, nesterov = True)
    model.compile(
...
model, history = fit_model([tsfeatures_train, w_train], y_train, 500, 8192)

##
model.summary()
model.save("../../models/bestmodel.h5")
...
fig.savefig("../../results/bestmodel.png")
fig.show()
